...
type responseGuide struct {
	GuideGif    string   `json:"guide_gif"`
	GuideStrArr []string `json:"guide_str_arr"`
}
...
func startRun(ctx iris.Context) {
	...
	println("")
	fmt.Printf("begin:%v,end:%v", plan.DateBegin, plan.DateEnd)
...
func finishRun(ctx iris.Context) {
	...
	//b2, err := json.MarshalIndent(record, "", "   ")
	//if err != nil {
	...
	fmt.Println("1111111111111111111111", invalidCode)
...
func guide(ctx iris.Context) {
	guideGif := "https://zthx.oss-cn-shanghai.aliyuncs.com/app/guide2.gif"
	guideStrArr := make([]string, 0)
	guideStrArr = append(guideStrArr, "1.请在光线适宜的情况下进行认证，尽量避免强光或过暗的环境")
	guideStrArr = append(guideStrArr, "2.请摘掉眼镜，不要遮挡人脸进行认证")
	guideStrArr = append(guideStrArr, "3.请您认证前确认开启存储权限和相机权限")
	guideStrArr = append(guideStrArr, "4.请您按照上述要求进行人脸认证，以免影响您的正常使用")

	guideResponse := responseGuide{
		GuideGif:    guideGif,
		GuideStrArr: guideStrArr,
	}

	ctx.JSON(lib.NewResponseOK(guideResponse))
}
