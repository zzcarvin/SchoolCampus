...
type responPlanProgress struct {
	...
	TotalDistance             int     `json:"total_distance" xorm:"-"`
	...
	Calories                  float32 `json:"-" xorm:"calories"`                 //进度总卡路里
	...
	ProgressTimes int `json:"progress_times" xorm:"-"`
}
...
type simplePlanProgress struct {
	...
	ScaleTotalDistance    int    `json:"scale_total_distance"`
	ScaleProgressDistance int    `json:"scale_progress_distance"`
}
...
func planProgress(ctx iris.Context) {
	...
	planAndProgress.TotalDistance = stuPlan.TotalTimes       //存疑
	planAndProgress.Distance = planAndProgress.TotalDistance //展示进度
...
func detailPlanProgress(ctx iris.Context) {
	...
	termPlanProgress.ScaleTotalDistance = plan.TotalDistance                             //总公里int类型
	termPlanProgress.Target = fmt.Sprintf("%d/%d次", planProgress.Times, plan.TotalTimes) //计划次数
	//去掉次数模式下的km单位
	termPlanProgress.TargetUnit = ""

	termPlanProgress.ScaleProgressDistance = planProgress.Distance //计划进度
	...
	for _, value := range records {
		...
		//weeRecords.ScaleProgressDistance = weeRecords.ScaleProgressDistance + value.Distance //本周公里数累计
	...
	println("本周累计跑步数据：")
	println("累计公里：", weeRecords.ScaleProgressDistance, "累计时长：", weekDuration, "累计步数：", weekSteps, "累计卡路里：", weekCalories)
	//固定数据获取
	...
	weeRecords.ScaleTotalDistance = plan.TotalDistance //总公里int类型
	//使用planProgress的周次数
	thisWeekTimes := planProgress.WeekTimes
	if planProgress.WeekTimes > plan.MinWeekTimes {
